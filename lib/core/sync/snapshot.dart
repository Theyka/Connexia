...
/// Serializes every syncable table into a [SyncSnapshotData].
Future<SyncSnapshotData> exportSnapshot(AppDatabase db) async {
  final settings = await db.allSettings();
  return SyncSnapshotData(
    hosts: (await db.allHosts()).map((h) => h.toJson()).toList(),
    groups: (await db.allGroups()).map((g) => g.toJson()).toList(),
    identities: (await db.allIdentities()).map((i) => i.toJson()).toList(),
    knownHosts: (await db.allKnownHosts()).map((k) => k.toJson()).toList(),
    snippets: (await db.allSnippets()).map((s) => s.toJson()).toList(),
...
/// Replaces the local database contents with the snapshot, atomically.
///
