...
class RenderTerminal extends RenderBox with RelayoutWhenSystemFontsChangeMixin {
  ...
  void _onScroll() {
    // Compare the raw pixel offset, not the line-snapped [_scrollOffset]:
    // snapping can round the position a few pixels away from the true max
    // extent, which made the viewport ratchet up and down on every layout
    // while sticking to the bottom (visible as jitter when typing into a
    // full scrollback buffer or while dragging a selection).
  ...
  void performLayout() {
    ...
    if (_stickToBottom) {
      // Correct to the exact max extent. Correcting against the snapped
      // [_scrollOffset] instead leaves the viewport a few pixels short of
      // the bottom (and oscillating there) whenever the content height is
      // not a whole multiple of the line height, which is the common case.
      // Painting still snaps to whole lines via [_scrollOffset].
  ...
  /// Snapped to whole lines so that lines are never painted at fractional
  /// y positions, which would leave the topmost visible line half-cut at
  /// the top edge of the viewport.
  double get _scrollOffset {
    final lineHeight = _painter.cellSize.height;
    if (lineHeight <= 0) return _offset.pixels;
    return (_offset.pixels / lineHeight).roundToDouble() * lineHeight;
  }
  ...
  void _applySelection(
    ...
    _controller.setSelection(base, extent, mode: mode);
