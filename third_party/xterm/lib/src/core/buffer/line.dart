...
class BufferLine with IndexedItem {
  ...
  void resize(int length) {
    ...
    if (length > _length) {
      ...
      // Cells beyond the old length must be blank. They can hold stale
      // content from an earlier wider state (a previous resize truncated
      // the line but kept the data); leaving them makes old characters
      // resurface and mix with a TUI's redraw after a narrow->wide toggle.
      for (var i = _length * _cellSize; i < length * _cellSize; i++) {
        _data[i] = 0;
      }
    }
