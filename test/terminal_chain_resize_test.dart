...
class _TitleBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const SizedBox(height: 40, child: ColoredBox(color: Colors.black));
  }
}
...
class _PaneState extends State<_Pane> {
  ...
  void _syncViewportSize(BuildContext context, Size size) {
    final painter = _cellPainter ??= TerminalPainter(
...
void main() {
  testWidgets('terminal pane follows window resize', (tester) async {
    ...
    expect(
      ...
      greaterThan(w0),
      reason:
          'growing the window must grow the terminal width '
          '(was $w0 -> $w1)',
    );
    expect(
      h1,
