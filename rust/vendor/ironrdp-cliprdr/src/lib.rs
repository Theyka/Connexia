...
use ironrdp_core::{AsAny, Decode as _, EncodeResult, IntoOwned as _, ReadCursor};
...
impl<R: Role> SvcProcessor for Cliprdr<R> {
    ...
    fn process(&mut self, payload: &[u8]) -> PduResult<Vec<SvcMessage>> {
        // Vendored change: a single channel payload can carry multiple
        // concatenated CLIPRDR PDUs (servers coalesce bursts of clipboard
        // messages). Decode in a loop instead of silently dropping everything
        // after the first PDU.
        let mut cursor = ReadCursor::new(payload);
        let mut messages = Vec::new();

        while !cursor.is_empty() {
            let pdu = ClipboardPdu::decode(&mut cursor).map_err(|e| decode_err!(e))?;
            messages.extend(self.process_clipboard_pdu(pdu)?);
        }
...
impl<R: Role> Cliprdr<R> {
    fn process_clipboard_pdu(&mut self, pdu: ClipboardPdu<'_>) -> PduResult<Vec<SvcMessage>> {
        match pdu {
            ...
            ClipboardPdu::FileContentsRequest(request) => {
                ...
                // Vendored change: when no tracked file list is available, do
                // NOT fail the request here. The crate's `PackedFileList`
                // model is unused by Connexia (the backend serves the file
                // descriptor from the OS clipboard and can likewise serve
                // contents itself), so forward every in-bounds request to the
                // backend and let it respond or error via
