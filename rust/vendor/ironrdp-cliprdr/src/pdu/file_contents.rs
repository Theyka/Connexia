...
impl<'de> Decode<'de> for FileContentsRequest {
    fn decode(src: &mut ReadCursor<'de>) -> DecodeResult<Self> {
        ...
        // Vendored change: upstream 0.7.0 enforces strict semantic validation
        // here (non-negative lindex, exactly one of SIZE/RANGE set, SIZE
        // requests with cbRequested=8 and position=0). Real servers send
        // mildly non-conforming requests that other clients (FreeRDP, mstsc)
        // accept, and rejecting them kills the whole session. Parse the PDU
        // and let the backend decide how to satisfy it; the backend responds
        // with an error when a request genuinely cannot be served.
        if index < 0 {
            tracing::warn!(index, "FileContentsRequest with negative lindex");
        }

        if flags.validate().is_err() {
            tracing::warn!(?flags, "FileContentsRequest with non-spec-conforming dwFlags");
        }

        if flags.contains(FileContentsFlags::SIZE) && requested_size != 8 {
            tracing::warn!(requested_size, "FileContentsRequest SIZE with cbRequested != 8");
        }
