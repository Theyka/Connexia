...
pub struct Config {
    ...
    /// Allow the legacy Standard RDP Security (`PROTOCOL_RDP`) handshake.
    ///
    /// IronRDP only implements the `ENCRYPTION_LEVEL_NONE` variant of
    /// Standard RDP Security: when enabled (and both `enable_tls` and
    /// `enable_credssp` are `false`), the client advertises `PROTOCOL_RDP`
    /// and, when the server selects it, proceeds without a TLS upgrade or
    /// CredSSP authentication. The RDP stream itself is not RC4-encrypted,
    /// so this only works against servers configured with no encryption
    /// (the server would otherwise reject the client's GCC security data).
    ///
    /// # Security Warning
    ///
    /// Do **not** enable this unless the transport is already protected.
    /// Without TLS, all RDP traffic — including credentials — is sent in
    /// plaintext.
