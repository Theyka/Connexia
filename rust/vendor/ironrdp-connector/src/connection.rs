...
pub struct ClientConnector {
    ...
    pub message_channel_id: Option<u16>,
    /// Security data advertised by the server in its GCC blocks.
    ///
    /// Only relevant for Standard RDP Security (`PROTOCOL_RDP`); `None` unless
    /// the server requested RDP-level encryption.
    pub server_security_data: Option<gcc::ServerSecurityData>,
}
        ...
        let (written, next_state) = match mem::take(&mut self.state) {
            ...
            ClientConnectorState::ConnectionInitiationSendRequest => {
                ...
                if security_protocol.is_standard_rdp_security()
                    && !self.config.enable_standard_rdp_security
                {
            ...
            ClientConnectorState::ConnectionInitiationWaitConfirm { requested_protocol } => {
                ...
                // PROTOCOL_RDP is an empty bitset, so `intersects` is false for
                // two standard-protocol sets. Treat it as a special case.
            ...
            // When PROTOCOL_RDP (standard RDP security) is selected there is no
            // TLS/CredSSP front-end; the caller still invokes
            // `mark_security_upgrade_as_done()` (a no-op upgrade) before continuing.
            ClientConnectorState::EnhancedSecurityUpgrade { selected_protocol } => {
            ...
            ClientConnectorState::BasicSettingsExchangeWaitResponse { connect_initial: _ } => {
                ...
                let server_uses_security = server_gcc_blocks.security != gcc::ServerSecurityData::no_security();

                if server_uses_security && !self.config.enable_standard_rdp_security {
                    return Err(general_err!("can't satisfy server security settings"));
                }

                // The server requested RDP-level encryption. The host is
                // responsible for driving the Standard RDP Security exchange
                // (Security Exchange PDU + RC4 key establishment) once the
                // channel connection phase completes.
                self.server_security_data = server_uses_security.then(|| server_gcc_blocks.security.clone());
...
fn create_gcc_blocks<'a>(
    ...
    Ok(ClientGccBlocks {
        ...
        security: ClientSecurityData {
            encryption_methods: if config.enable_standard_rdp_security {
